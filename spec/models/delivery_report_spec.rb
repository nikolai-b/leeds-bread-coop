describe DeliveryReport do
  ...
  before do
    ...
    create :subscription, subscriber: unpaid_subscriber, paid_till: nil
  ...
  describe '#show' do
    ...
    context 'on Thursday' do
      let(:date) { Date.today.next_week.advance(days: 17) }
      ...
      it "each delivery has the collection point, bread_type, and subscriber name" do
        ...
        expect(first_delivery.collection_point.name).to eq('Green Action Food Co-op')
    ...
    context 'on Thursday' do
      ...
      context 'a subscriber with more bread_types than paid subs' do
        before do
          ...
          create :subscription, bread_type: bread_type, paid_till: nil
        end
